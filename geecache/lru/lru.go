...
type Cache struct {
	...
	cacheMap       map[string]*list.Element      // 存储缓存的键值映射关系
...
func (c *Cache) Add(key string, value Value) {
	if listele, ok := c.cacheMap[key]; ok {
		// 如果键存在则更新对应节点的值，并将该节点移到队尾
		c.doubleLinkList.MoveToFront(listele)
		kv := listele.Value.(*listNode)
		c.nowBytes += int64(value.Len()) - int64(kv.value.Len())
		kv.value = value
	} else {
		// 不存在则新增，在队尾添加新节点
		listele := c.doubleLinkList.PushFront(&listNode{key, value})
		// 在字典中添加 key 和节点的映射关系
...
func (c *Cache) Get(key string) (value Value, ok bool) {
	if listele, ok := c.cacheMap[key]; ok {
		// 将链表中的节点 listele 移动到队尾（双向链表作为队列，队首队尾是相对的，在这里约定 front 为队尾）
		c.doubleLinkList.MoveToFront(listele)
		kv := listele.Value.(*listNode)
...
func (c *Cache) RemoveOldest() {
	// 取到队首节点
	listele := c.doubleLinkList.Back()
	if listele != nil {
		// 从链表中删除
		c.doubleLinkList.Remove(listele)
		kv := listele.Value.(*listNode)
		// 从字典中删除节点的映射关系
